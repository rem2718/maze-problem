...
class MazeEnv(gym.Env):
    ...
    def _transition(self, s, a):
        ...
        next_s = np.clip( s + dir, 0, self.size - 1)
        return next_s
    ...
    def step(self, a):
        loc = self._transition(self._agent_location, a) 
        reward = self._get_reward(loc)
    ...
    def _render_frame(self):
        ...
        for row in range(self.size):
            for col in range(self.size):
                ...
                if np.isnan(value):
                    continue
                text = self.font.render(str(round(value, 1)), True, (255, 255, 255))  
